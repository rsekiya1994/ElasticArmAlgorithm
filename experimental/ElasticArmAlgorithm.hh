...
namespace rse {
  template <class T>
  class ElasticArmAlgorithmImpl;

  template <class T>
  using ElasticArmAlgorithm = ElasticArmAlgorithmImpl<T>;
}


template <class T>
class rse::ElasticArmAlgorithmImpl : public rse::QuasiNewton<rse::ElasticArmAlgorithmImpl<T>> {
  public:
  ElasticArmAlgorithmImpl(int dim, int nPar) : rse::QuasiNewton<rse::ElasticArmAlgorithmImpl<T>>(nPar), params(nPar), dim_(dim) {}
  virtual ~ElasticArmAlgorithmImpl() {};
  ...
  template <class U>
  inline void AddPoints(const U &vec, double lambda) {
  ...
  inline double GetFreeEnergy() { return this->Func(params); }
   ...
   */
  double Distance(const Eigen::VectorXd &data_point, const Eigen::VectorXd &params);

  double Constraint(const Eigen::VectorXd &params) {
    return 0;
  }

  inline void SetTemperature(double temperature) { temperature_ = temperature; }

  public:
  inline double Func(const Eigen::VectorXd &params_);
 ...
 */
template <class T>
inline double rse::ElasticArmAlgorithmImpl<T>::Func(const Eigen::VectorXd &params_) {
  ...
  for (int ipoint = 0; ipoint < data_size; ++ipoint) {
    auto distance = static_cast<T *>(this)->Distance(data_points[ipoint], params_) / lambdas_[ipoint];
  ...
  free_energy += beta_ * static_cast<T *>(this)->Constraint(params_);
  return free_energy;
}

template <class T>
inline Eigen::VectorXd rse::ElasticArmAlgorithmImpl<T>::NablaF(const Eigen::VectorXd &params_) {
  double beta_ = 1. / temperature_;
  int data_size = data_points.size();
  ...
  for (std::size_t ipar = 0; ipar < nPar_; ++ipar) {
    for (int ipoint = 0; ipoint < data_size; ++ipoint) {
      double distance = static_cast<T *>(this)->Distance(data_points[ipoint], params_) / lambdas_[ipoint];
      double weight   = 1. / (1 + std::exp(beta_ * (distance - 1)));
      h_[ipar]      = dx__;
      double fw_    = static_cast<T *>(this)->Distance(data_points[ipoint], params_ + h_) / lambdas_[ipoint];
      double bw_    = static_cast<T *>(this)->Distance(data_points[ipoint], params_ - h_) / lambdas_[ipoint];
  ...
  for(std::size_t ipar = 0; ipar < nPar_; ++ipar) {
    h_[ipar] = dx__;
    double fw_ = static_cast<T *>(this)->Constraint(params_ + h_);
    double bw_ = static_cast<T *>(this)->Constraint(params_ - h_);
...
template <class T>
inline bool rse::ElasticArmAlgorithmImpl<T>::ProcessEAA(const std::vector<double> &init_params, double nMaxLoop, double epsilon) {
...
template <class T>
inline double rse::ElasticArmAlgorithmImpl<T>::GetProb(int ipoint) {
  auto distance = static_cast<T *>(this)->Distance(data_points[ipoint], params) / lambdas_[ipoint];
  auto beta     = 1. / temperature_;
  return 1. / (1 + std::exp(beta * (distance - 1)));
}

template <class T>
inline double rse::ElasticArmAlgorithmImpl<T>::GetDistance(int ipoint) {
  return static_cast<T *>(this)->Distance(data_points[ipoint], params);
}
 ...
 */
template <class T>
inline double rse::ElasticArmAlgorithmImpl<T>::GetEnergy() {
  double result = 0;
  int nPoints = data_points.size();
  for(int ipoint = 0; ipoint < nPoints; ++ipoint) {
    auto distance = static_cast<T *>(this)->Distance(data_points[ipoint], params);
  ...
  return result + static_cast<T *>(this)->Constraint(params);
}